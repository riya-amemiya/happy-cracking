...
pub fn encode(input: &str, alphabet: &str) -> Result<String> {
    ...
    // undefined behavior if a future refactoring accidentally breaks the ASCII-only invariant.
    Ok(String::from_utf8(bytes).expect("substitution of ASCII letters must produce valid UTF-8"))
}
...
fn apply_key(input: &str, key: &[u8; 26]) -> String {
    ...
    // Security: use safe String::from_utf8 instead of unsafe from_utf8_unchecked to prevent
    // undefined behavior if a future refactoring accidentally breaks the ASCII-only invariant.
    String::from_utf8(bytes).expect("substitution of ASCII letters must produce valid UTF-8")
}
