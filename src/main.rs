...
#[command(
    after_help = "This crate also installs hgrep, a parallel grep-compatible line matcher, and hfind, a parallel find-compatible walker. hg and hrg are aliases for hgrep that search directories like rg. hfd is an alias for hfind. Run hgrep --help or hfind --help for usage."
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    ...
    #[command(about = "Generate strings quickly (random, exhaustive, or masked)")]
    Strgen {
        #[command(subcommand)]
        action: crypto::strgen::StrgenAction,
    },
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Strgen { action } => crypto::strgen::run(action)?,
