...
## 2026-08-28 - Semaphore leftover decode HashMap
**Learning:** After encode used `ENCODE_LUT`, decode still hashed every `"X-Y"` token in `LazyLock<HashMap<&str, char>>` and collected chars without a reserved buffer. Flag semaphore is two digits 1–8 (order matters: `1-3`≠`3-1`), so a 9×9 table is an exact fit.
**Action:** Compile-time `DECODE_LUT: [[Option<char>; 9]; 9]`. Parse the 3-byte `"X-Y"` form, index `[first][second]`, push into one pre-sized `String`. Drop `HashMap`/`LazyLock`. Const-assert all 26 map rows landed. ~2.4x HELLO, ~3.0x alphabet, ~3.1x on ~4 KiB.
