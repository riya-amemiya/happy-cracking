...
## 2026-03-05 - Missing BSGS limit in DH implementation allows DoS
...
## 2026-04-16 - BigUint Subtraction Panic in RSA compute-d
**Vulnerability:** `RsaAction::ComputeD` computed `phi = (p - 1) * (q - 1)` directly on the user-supplied `BigUint` values. `BigUint` subtraction panics when the result would be negative, so passing `p = 0` or `q = 0` via CLI (`rsa compute-d --p 0 --q 53 --e 17`) crashed the process with an unhandled panic \u2014 a DoS vector.
**Learning:** The existing zero-modulus guards in `big_modinv` / `big_modpow` are not sufficient when preceding BigUint arithmetic can panic before the guarded call. Every arithmetic operation on user-supplied `BigUint` needs its own precondition checked at the public entry point, because the panic happens inside `num-bigint`, not inside our Result-returning helpers.
**Prevention:** When exposing arithmetic on unsigned big integers to CLI input, validate each operand before the first subtraction / division that would underflow or divide by zero. Extract a testable `pub fn compute_d(p, q, e) -> Result<BigUint>` so regression tests can cover the zero-guard directly instead of going through the `run` action dispatcher.
